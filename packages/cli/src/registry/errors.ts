			...
			? `Unknown registry "${registryName}". Make sure it is defined in agents.json as follows:
{
  "registries": {
    "${registryName}": "[URL_TO_REGISTRY]"
			...
			: `Unknown registry. Make sure it is defined in agents.json under "registries".`

		super(message, {
			code: RegistryErrorCode.NOT_CONFIGURED,
			context: { registryName },
			suggestion:
				"Add the registry configuration to your agents.json file. Consult the registry documentation for the correct format.",
		})
...
export class ConfigMissingError extends RegistryError {
	constructor(public readonly cwd: string) {
		const message = `No agents.json found in ${cwd} or parent directories.`

		super(message, {
			code: RegistryErrorCode.NOT_CONFIGURED,
			context: { cwd },
			suggestion:
				"Run 'awesome-ai init' to create an agents.json file, or check that you're in the correct directory.",
		})
...
export class ConfigParseError extends RegistryError {
	constructor(
		...
		let message = `Invalid agents.json configuration in ${cwd}.`

		if (parseError instanceof z.ZodError) {
			message = `Invalid agents.json configuration in ${cwd}:\n${parseError.issues
				.map((e) => `  - ${e.path.join(".")}: ${e.message}`)
				.join("\n")}`
		}
		...
		super(message, {
			...
			context: { cwd },
			suggestion:
				"Check your agents.json file for syntax errors or invalid configuration. Run 'awesome-ai init' to regenerate a valid configuration.",
		})
