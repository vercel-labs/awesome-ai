...
// Increase timeout for all tests in this file as ripgrep has initialization overhead
...
describe("globTool", () => {
	...
	it(
		"returns no files found message when no matches",
		async () => {
			await fs.writeFile(path.join(tempDir, "file.txt"), "content")
	...
	it(
		"yields pending status before completion",
		async () => {
			await fs.writeFile(path.join(tempDir, "file.txt"), "content")
	...
	it(
		"defaults to current directory when path not provided",
		async () => {
			const results = await executeTool(globTool, {
	...
	it(
		"matches files with specific names",
		async () => {
